...
from model import VGG19, CameraEncoder, ShapeEncoder, LightEncoder, TextureEncoder

torch.autograd.set_detect_anomaly(True)

parser = argparse.ArgumentParser()
parser.add_argument('--outf',default='./output', help='folder to output images and model checkpoints')
parser.add_argument('--dataroot',default='./', help='path to dataset root dir')
...
class AttributeEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            self.feat_enc.eval()
            img_feats = self.feat_enc(img_pro)
    ...
    for epoch in range(start_epoch, opt.niter):
        for iter, data in enumerate(train_dataloader):
            with Timer("Elapsed time in update: %f"):
                ...
                Xa_ = data['data']['images']
                Xa = Variable(Xa_).cuda()
                ...
                if opt.lambda_ic > 0.0:
                    ...
                    Ai['azimuths'] = - torch.empty((batch_size), dtype=torch.float32).uniform_(-opt.azi_scope / 2,
                                                                                               opt.azi_scope / 2).cuda()
                ...
                lossR_flip = 0.1 * (
                            diffRender.recon_flip(Ae) + diffRender.recon_flip(Ai) + diffRender.recon_flip(Aire)) / 3.0
        ...
        if epoch % 20 == 0 and epoch > 0:
            ...
            for i, data in tqdm.tqdm(enumerate(test_dataloader)):
                ...
                with torch.no_grad():
                    ...
                    Ai = deep_copy(Ae)
                    Ai['azimuths'] = - torch.empty((Xa.shape[0]), dtype=torch.float32).uniform_(-opt.azi_scope / 2,
                                                                                                opt.azi_scope / 2).cuda()
                    Xir, Ai = diffRender.render(**Ai)
